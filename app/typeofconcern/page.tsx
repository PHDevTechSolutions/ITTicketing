...
export default function ConcernTypesPage() { 
  ...
  const handleSaveConcernType = async () => { 
    ...
    try {
      ...
      if (data.success) {
        alert(`Type of Concern ${currentConcernType ? "updated" : "created"} successfully!`);
        fetchConcernTypes(); 
      } else {
          ...
          <div className="flex items-center gap-4">
            ...
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbItem>
                  <BreadcrumbPage className="dark:text-zinc-100">Type of Concern</BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-left text-xs">
                <thead className="bg-gray-700 dark:bg-zinc-800 text-white font-semibold sticky top-0">
                  <tr>
                    <th className="p-4">Type of Concern Name</th>
                ...
                <tbody className="divide-y dark:divide-zinc-800">
                  {isLoading ? (
                    ...
                    concernTypes.map((type) => (
                      <tr key={type._id} className="border-b dark:border-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition-colors">
                        <td className="p-4 font-medium text-gray-800 dark:text-zinc-200">{type.name}</td>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="dark:bg-zinc-900 dark:border-zinc-800">
                              ...
                              <DropdownMenuItem
                                ...
                                className="text-gray-700 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800 cursor-pointer"
                              ><Edit className="mr-2 h-4 w-4" /> Edit</DropdownMenuItem>
                              <DropdownMenuItem
                                onClick={() => handleDelete(type._id, type.name)}
                                className="text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 cursor-pointer"
                    ...
                    <tr>
                      <td colSpan={2} className="p-6 text-center text-gray-500 dark:text-zinc-500 italic">
                        No concern types found. Click "+ Type of Concern" to add one.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <DialogContent className="sm:max-w-[425px] dark:bg-zinc-950 dark:border-zinc-800">
            ...
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="concernTypeName" className="text-right text-sm font-medium dark:text-zinc-300">Name :</Label>
                <Input
                  id="concernTypeName"
            ...
            <DialogFooter>
              <DialogClose asChild><Button variant="outline" className="dark:border-zinc-800 dark:text-zinc-300" disabled={isSaving}>Cancel</Button></DialogClose>
              <Button onClick={handleSaveConcernType} className="bg-gray-700 hover:bg-gray-800 dark:bg-zinc-100 dark:text-zinc-950" disabled={isSaving}>
                {isSaving ? (
